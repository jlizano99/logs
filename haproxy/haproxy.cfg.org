...
#acl is_odds_sitemaps path_reg -i ^/(sitemap-league-.*\.xml|sitemap-team-.*\.xml|sitemap-scores-and-matchups.*\.xml)
...
backend csl.cluster
        ...
        server 172.22.3.240-nginx 172.22.3.240:86 cookie A check
...
backend picks.cluster
        ...
        option allbackups
        server 172.22.3.240-nginx 172.22.3.240:83 check
...
backend sbrodds.cluster
        ...
        cookie JSESSIONID prefix
        option allbackups
        server 172.22.3.240-nginx 172.22.3.240:87

backend sportsbookreview.cluster
        mode http
        balance roundrobin
        cookie JSESSIONID prefix
        option httpchk HEAD /asg.txt HTTP/1.0\r\nHOST:\ www.sportsbookreview.com
        option allbackups
        server 172.22.3.240-varnish 172.22.3.240:6081 check
        server 172.22.3.240-nginx 172.22.3.240:81 cookie A check backup

backend microservices.cluster
        mode http
        balance roundrobin
        server 172.22.3.240-nginx 172.22.3.240:89 check

backend posting_forum.cluster
        mode http
        balance roundrobin
        cookie JSESSIONID prefix
        option httpchk HEAD /forum.txt HTTP/1.0
        server 172.22.3.240-nginx 172.22.3.240:84 cookie A check

#########################################################################################

backend best_sportsbooks.cluster
        mode http
        balance roundrobin
        cookie JSESSIONID prefix
        # option httpchk HEAD /forum.txt HTTP/1.0
        server localhost-nginx localhost:91 cookie A

backend sportsbook_casino.cluster
        mode http
        server 172.22.3.240 172.22.3.240:82 check

backend livehelpsportsbookreview.cluster
        mode http
        balance roundrobin
        server 172.22.4.45 172.22.4.45:80 cookie A check

backend adn.sbr.cluster
        mode http
        balance roundrobin
        cookie JSESSIONID prefix
        errorfile 503 /etc/haproxy/503-200.http
        option httpchk HEAD /adn.txt HTTP/1.0\r\nHOST:\ adn.sportsbookreview.com
        server 172.22.3.240-nginx 172.22.3.240:85 cookie A check

backend mysbrforum.cluster
        mode http
        balance roundrobin
        cookie JSESSIONID prefix
        server 172.22.3.240-nginx 172.22.3.240:88 cookie A check
